...
$textAreaLabel = array('Idéia da aventura');
$textAreaText = array('texto');


$pagina_nome = 'Aventuras.';
$descriacao= array('create'=>'Criação de uma nova idéia de aventura no Help Rpg.',
				   'edit'=>'Editando uma aventura no Help Rpg.',
				   'destroy'=>'Apagando uma aventura no Help Rpg.');
...
switch ($tela):
	case 'incluir':
		...
		if(isset($_POST['cadastrar'])):
			...
			if($objeto->linhas_afetadas == 1):
				printMsg('Aventura criada com sucesso <a href="?m='.$modulo.'&t=listar">Exibir cadastros</a>');
				unset($_POST);
			endif;			
		endif;	
	?>
	...
	case 'listar':
		...
		$tag->open('table','cellspacing="0" cellpadding="0" border="0" class="display" id="listausers" ');
			$tag->open('thead');
			 	$tag->open('tr');
                    $labels = array('Título','Históris','Ações');
			 		ths($labels);
			 	$tag->close('tr');
		...
		if(isAdmin() == TRUE || $sessao->getVar('id_user') == $_GET['id']):
			if(isset($_GET['id'])):
				...
				if(isset($_POST['editar'])):
					...
					if($objeto->linhas_afetadas == 1):
						printMsg('Aventura editada com sucesso. <a href="?m='.$modulo.'&t=listar">Exibir história de aventura</a>');
						unset($_POST);
					else:
						printMsg('Nenhum dado foi alterado. <a href="?m='.$modulo.'&t=listar">Exibir história de aventura</a>','alerta');	
					endif;
				endif;				
