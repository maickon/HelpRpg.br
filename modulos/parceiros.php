...
$inputLabel1 = array('Nome');
$inputName1 = array('nome');

$inputFileLabel = 'Logo';
$inputFileName = 'arquivo1';

$inputLabel2 = array('Link de referência');
$inputName2 = array('link');



$pagina_nome = 'Configuração da página de Parceiros do Help Rpg.';
$descriacao= array('create'=>'Criação da página Parceiros do Help Rpg.',
				   'edit'=>'Editando a página Parceiros do Help Rpg.',
				   'destroy'=>'Apagando a página de um Parceiro do Help Rpg.');
...
switch ($tela):
	case 'incluir':
		...
		if(isset($_POST['cadastrar'])):
			$objeto = new $classe(array(
				"nome" => $_POST['nome'],
				"img1" => gerar_nome($_FILES['arquivo1']['name']),
				"link" => $_POST['link']
			));
		
			$objeto->inserir($objeto);
			if($objeto->linhas_afetadas == 1):
				upload_files($objeto,PARCEIROSPATH);
    ...
    $tag->open('div','class="span12"');
	    $tag->open('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');
	        $tag->open('fieldset');
				...
				input_file($inputFileLabel, $inputFileName);
	...
	case 'listar':
		...
		$tag->open('table','cellspacing="0" cellpadding="0" border="0" class="display" id="listausers" ');
			$tag->open('thead');
			 	$tag->open('tr');
                    $labels = array('Nome','Link de referência','Ações');
			 		ths($labels);
			 	$tag->close('tr');
		...
		if(isAdmin() == TRUE || $sessao->getVar('id_user') == $_GET['id']):
			if(isset($_GET['id'])):
				...
				if(isset($_POST['editar'])):
					...
					if($objeto->linhas_afetadas == 1):
						delete_files($objeto,PARCEIROSPATH);
						upload_files($objeto,PARCEIROSPATH);
		...
		<?php
		 $tag->open('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');
			$tag->open('fieldset');
				...
				$inputValuesEdit1 = array($objeto_resp->nome);
				$inputValuesEdit2 = array($objeto_resp->link);
				input_file($inputFileLabel, $inputFileName);
		...
		if(isAdmin() == TRUE || $sessao->getVar('id_user') == $_GET['id']):
			if(isset($_GET['id'])):
				...
				if(isset($_POST['excluir'])):
					...
					if($objeto->linhas_afetadas == 1):
						delete_files($resp,PARCEIROSPATH);
		 ...
		 $tag->open('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');
			$tag->open('fieldset');
				...
				$inputValuesEdit1 = array(limpar_campo_excluir('nome', $objeto_resp));
				$inputValuesEdit2 = array(limpar_campo_excluir('link', $objeto_resp));
				input_file($inputFileLabel, $inputFileName,null,'disabled');
