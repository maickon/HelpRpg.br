...
$tag = new tags();
$objeto = 'historias_page';
$classe = 'hist_page';
$modulo = 'historias_page';

$inputLabel = array('Título');
$inputName = array('titulo');

$selectLabes = array('Domínio?');
$selectNames = array('dominio');
$selectOptions = array('publico','privado');
...
switch ($tela):
	...
	case 'listar':
		...
		$tag->open('table','cellspacing="0" cellpadding="0" border="0" class="display" id="listausers" ');
			$tag->open('thead');
			 	$tag->open('tr');
                    $labels = array('Título','Domínio','Páginas','Ações');
			 		ths($labels);
			 	$tag->close('tr');
