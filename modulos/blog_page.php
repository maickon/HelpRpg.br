...
$inputLabel1 = array('Título');
$inputName1 = array('titulo');

$inputLabel2 = array('Categoria');
$inputName2 = array('categoria');

$textAreaLabel = array('Minha página');
$textAreaText = array('texto');


$pagina_nome = 'Configuração da Blog Page.';
$descriacao= array('create'=>'Criação da página principal do Blog Rpg.',
				   'edit'=>'Editando a página principal do Blog Rpg.',
				   'destroy'=>'Apagando a página principal do Blog Rpg.');
...
switch ($tela):
	...
	case 'listar':
		...
		$tag->open('table','cellspacing="0" cellpadding="0" border="0" class="display" id="listausers" ');
			$tag->open('thead');
			 	$tag->open('tr');
                    $labels = array('Título','Categoria','Páginas','Ações');
			 		ths($labels);
			 	$tag->close('tr');
		...
		<?php
		 $tag->open('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');
			$tag->open('fieldset');
				...
				$inputValuesEdit = array($objeto_resp->titulo,$objeto_resp->categoria);
				input($inputLabel1, $inputName1,$inputValuesEdit);
				input($inputLabel2, $inputName2,$inputValuesEdit);
		...
		if(isAdmin() == TRUE || $sessao->getVar('id_user') == $_GET['id']):
		 ...
		 $tag->open('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');
			$tag->open('fieldset');
				...
				$inputValuesEdit = array(limpar_campo_excluir('titulo',$objeto_resp),limpar_campo_excluir('categoria',$objeto_resp));
				input($inputLabel1, $inputName1,$inputValuesEdit,'disabled');
				input($inputLabel2, $inputName2,$inputValuesEdit,'disabled');
