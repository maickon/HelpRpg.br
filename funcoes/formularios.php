...
function input($labels, $names, $value='', $disabled='',$type='text', $id=null, $class=null){
	$tag = new tags();
	for($i=0; $i<count($names); $i++):
		...
		if($value == ''):
			$tag->open('input',''.$_id.''.$_class.' type="'.$type.'" '.$disabled.' size="50" name="'.$names[$i].'"');
		else:
			$tag->open('input',''.$_id.''.$_class.' type="'.$type.'" '.$disabled.' size="50" name="'.$names[$i].'" value="'.$value[$i].'"');
		endif;
	endfor;
}
...
function botoesCrud($object,$modulo,$cliente=false){
	$tag = new tags();
	$tag->open('td','class="center"');
		$tag->open('div');
			...
			if($cliente):
				senhaAventureiro($object, $modulo);
			endif;
		$tag->close('div');
	$tag->close('td');
}

function senhaAventureiro($object,$modulo){
	$tag = new tags();
	$tag->open('a','href="?m='.$modulo.'&t=senha&id='.$object.'" title="Alterar senha"');
		$tag->open('img','src="img/pass.png" alt="Alterar senha"','/');
	$tag->close('a');		
}
