...
function anuncio_menu($categoria='Categorias',$parceiros='Parceiros &rarr;',$mais='Conheça esse galea ae!'){
	$tag = new tags();
	$tag->close('div');			
		...
		$tag->open('div','class="well parceiros" align="left"');
			...
			$tag->open('p');
				...
				$parceiro->extras_select = "ORDER BY RAND() ";
				$parceiro->seleciona_tudo($parceiro);
				while($resp = $parceiro->retorna_dados()):
					$tag->open('a','href="http://'.$resp->link.'" target="_blank"');
						$tag->open('img','src="'.PARCEIROSPATH.$resp->img1.'" class="img-polaroid img-logo" alt="'.$resp->nome.'" title="Clique aqui e conheça: '.$resp->nome.'! Com certeza você não vai se arrepender."');
					$tag->close('a');	
				endwhile;
			$tag->close('p');
		$tag->close('div');
